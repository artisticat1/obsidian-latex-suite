...
import {
	sortSnippets,
	getSnippetsFromString,
	isInFolder,
	snippetInvertedEffects,
	debouncedSetSnippetsFromFileOrFolder,
} from "./snippets/snippet_helper_functions";
import { SnippetManager } from "./snippets/snippet_manager";
import {
	markerStateField,
	startSnippet,
	undidEndSnippet,
} from "./snippets/marker_state_field";
...
export default class LatexSuitePlugin extends Plugin {
	...
	async onload() {
		...
		this.registerEvent(
			this.app.vault.on("delete", (file) => {
				const snippetDir = this.app.vault.getAbstractFileByPath(
	...
	async onFileChange(file: TFile) {
		...
		if (
			file.path === this.settings.snippetsFileLocation ||
			this.fileIsInSnippetsFolder(file)
		) {
			try {
	...
	private readonly handleCursorActivity = (view: EditorView, pos: number) => {
		...
		if (
			!this.snippetManager.isInsideATabstop(pos) ||
			this.snippetManager.isInsideLastTabstop(view)
		) {
	...
	async loadSettings() {
		...
		this.setAutofractionExcludedEnvs(
			this.settings.autofractionExcludedEnvs
		);
		this.matrixShortcutsEnvNames = this.settings.matrixShortcutsEnvNames
			.replace(/\s/g, "")
			.split(",");
		this.autoEnlargeBracketsTriggers =
			this.settings.autoEnlargeBracketsTriggers
				.replace(/\s/g, "")
				.split(",");
	...
	private readonly handleKeydown = (
		...
		if (withinEquation)
			withinMath = !isInsideEnvironment(view, pos, {
				openSymbol: "\\text{",
				closeSymbol: "}",
			});
		...
		const shouldTaboutByCloseBracket = this.shouldTaboutByCloseBracket(
			view,
			key
		);
	...
	private readonly runSnippetCursor = (
		view: EditorView,
		key: string,
		withinMath: boolean,
		range: SelectionRange
		...
		for (const snippet of this.snippets) {
			...
			const result = this.checkSnippet(
				snippet,
				effectiveLine,
				range,
				sel
			...
			if (snippet.options.contains("w")) {
				...
				const prevChar = view.state.sliceDoc(
					triggerPos - 1,
					triggerPos
				);
				...
				const prevCharIsWordDelimiter =
					wordDelimiters.contains(prevChar);
				const nextCharIsWordDelimiter =
					wordDelimiters.contains(nextChar);
			...
			this.snippetManager.queueSnippet({
				from: start,
				to: to,
				insert: replacement,
				keyPressed: key,
			...
			const containsTrigger = this.autoEnlargeBracketsTriggers.some(
	...
	private readonly runAutoFraction = (
		view: EditorView,
		ranges: SelectionRange[]
	): boolean => {
		for (const range of ranges) {
	...
	private readonly runAutoFractionCursor = (
		...
		if (from != to) {
			...
			for (let i = curLine.length - 1; i >= eqnStart; i--) {
				...
				if (
					" $([{\n"
						.concat(this.settings.autofractionBreakingChars)
						.contains(curChar)
				) {
					start = i + 1;
					break;
	...
	private readonly tabout = (
		view: EditorView,
		withinEquation: boolean
	): boolean => {
	...
	private readonly runMatrixShortcuts = (
		view: EditorView,
		key: string,
		shiftKey: boolean,
		pos: number
	...
	private readonly shouldTaboutByCloseBracket = (
		view: EditorView,
		keyPressed: string
	) => {
